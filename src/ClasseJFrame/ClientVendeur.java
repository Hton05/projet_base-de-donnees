...
public class ClientVendeur extends JFrame {
	...
	public ClientVendeur(String total, String nature, ArrayList <String> listArticle, ArrayList <Integer> listQuantite) {
		...
		JLabel lblGestionArticle = new JLabel("CLIENT");
		...
		JButton btnNewButton_1 = new JButton("Placer commande");
		btnNewButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				DateFormat format = new SimpleDateFormat("yyyy-MM-dd");
				Date date = new Date();
				if(nature.equals("Facture")) {
					String[] colonne = {"date_commande", "date_livraison", "montant", "nature", "idClient"};
					String[] contenu = {format.format(date), format.format(date), total,"Facture", String.valueOf(table_Client.getValueAt(table_Client.getSelectedRow(), 0))};
					db.insert("commande", colonne, contenu);
					
				} else {
					String[] colonne = {"date_commande", "montant", "nature", "idClient"};
					String[] contenu = {format.format(date), total, "Proforma", String.valueOf(table_Client.getValueAt(table_Client.getSelectedRow(), 0))};
					db.insert("commande", colonne, contenu);
				}
				rs = db.selectAll("commande");
				try {
					while (rs.next()) {
						if(rs.isLast()) {
							String idCom = rs.getString("idCommande");
							String[] colonne1 = {"idCommande", "id_Article", "qte_article"};
							for(int i = 0; i < listArticle.size(); i++) {
								String[] contenu1 = {rs.getObject(1).toString(), listArticle.get(i).toString(), listQuantite.get(i).toString()};
								db.insert("article_commande", colonne1, contenu1);
								String query = "UPDATE article SET qte_en_stock = qte_en_stock - " +listQuantite.get(i)+ " WHERE id_Article = " +listArticle.get(i);
		...
		btnNewButton_1.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseEntered(MouseEvent e) {
				btnNewButton_1.setBackground(Color.decode("#EAECEE"));
				btnNewButton_1.setFont(new Font("Noto Sans CJK JP", Font.BOLD, 22));
			}
			@Override
			public void mouseExited(MouseEvent e) {
		...
		btnNewButton_1.setIcon(new ImageIcon(this.getClass().getResource("/add6.png")));
		btnNewButton_1.setFont(new Font("Noto Sans CJK JP", Font.BOLD, 20));
		btnNewButton_1.setBackground(couleur_back);
		btnNewButton_1.setForeground(couleur2);
		btnNewButton_1.setBounds(240, 577, 286, 42);
